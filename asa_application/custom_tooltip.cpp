...
void custom_tooltip::init_data()
{
    ...
    for(i = 0; i < (quint32)DataList.size(); i++)
    {
        ...
        {
            label = new QListWidgetItem(getParamName(param_id) + ": " + getParamValue_and_units(param_id));
...
//        list_widget->addItem(NameList[i]  + ": " + getParamValue_and_units(param_id));
        list_widget->addItem(getParamName(param_id)  + ": " + getParamValue_and_units(param_id));
